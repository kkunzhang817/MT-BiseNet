...
def parse_args():
    ...
    parser.add_argument('--backbone', type=str, default='resnet50',
                        choices=['vgg16', 'resnet18', 'resnet50', 'resnet101', 'resnet152', 
                            'densenet121', 'densenet161', 'densenet169', 'densenet201', 'xception'],
                        help='backbone name (default: vgg16)')
    parser.add_argument('--dataset', type=str, default='pascal_voc',
                        choices=['pascal_voc', 'pascal_aug', 'ade20k', 'citys', 'sbu', 'leaf'],
                        help='dataset name (default: pascal_voc)')
    parser.add_argument('--base-size', type=int, default=800,
    ...
    parser.add_argument('--warmup-iters', type=int, default=0,
                        help='warmup iters')
    parser.add_argument('--warmup-factor', type=float, default=1.0 / 2,
...
class Trainer(object):
    def __init__(self, args):
        ...
        # self.lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, mode='max', factor=0.1, patience=2,
        #                                                        verbose=True, threshold=0.001, threshold_mode='rel',
        #                                                        cooldown=2, min_lr=0, eps=1e-30)
        if args.distributed:
    ...
    def train(self):
        ...
        for epoch in range(epochs):
            tot_loss = 0
            for iteration, (images, targets, _) in enumerate(tqdm.tqdm(self.train_loader,desc="Train epoch:{}".format(str(epoch)))):
                ...
                mean_loss = tot_loss / iteration
                # if iteration % log_per_iters == 0 and save_to_disk:

            logger.info(
                " Epoch: {:d} || Lr: {:.6f} || Loss: {:.4f} || Cost Time: {} ".format(
                    epoch,  self.optimizer.param_groups[0]['lr'], mean_loss,
    ...
    def validation(self,epoch):
        ...
        totel_pixAcc = 0
        totel_mIoU = 0
        for i, (image, target, filename) in enumerate(tqdm.tqdm(self.val_loader, desc="Val epoch:{}".format(str(epoch)))):
            i = i + 1
            ...
            totel_pixAcc +=  pixAcc
            totel_mIoU += mIoU
            mean_pixAcc = totel_pixAcc / i
            mean_mIou  = totel_mIoU / i
        logger.info("Epoch: {:d}, Validation pixAcc: {:.3f}, mIoU: {:.3f}".format(epoch, mean_pixAcc, mean_mIou))
